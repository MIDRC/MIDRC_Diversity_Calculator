from PySide6.QtWidgets import (QWidget, QMenu, QFileDialog, QApplication, QDialog, QVBoxLayout, QHBoxLayout,
                               QPushButton, QLabel)
from PySide6.QtGui import QClipboard, QAction, QImage, QPixmap
...
class GrabbableWidgetMixin(QObject):
    ...
    def saveHighResToDisk(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self.parent, "Save High Resolution Snapshot", default_filename + "_highres.png",
                                                  ...
                                                  options=options)
        if fileName:
            # Capture the current widget's content
            snapshot = self.parent.grab(QRect(QPoint(0,0),self.parent.size() * 2))
            # Scale the snapshot to a higher resolution
            high_res_snapshot = snapshot.scaled(snapshot.width() * 2, snapshot.height() * 2, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            high_res_snapshot.save(fileName)
            print(f"High resolution snapshot saved to {fileName}")
